...
export function updateStashTokens(
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (action === "add") {
      ...
      if (!tokensDelegatedId.includes(tokens[i])) {
        ...
        tokensDelegatedAmount[index] = tokensDelegatedAmount[index].plus(
          amounts[i]
        );
      ...
      tokensDelegatedAmount[index] = tokensDelegatedAmount[index].minus(
        amounts[i]
      );
  ...
  for (let i = 0; i < tokensDelegatedId.length; i++) {
    ...
    if (temp == BIGINT_ZERO && i == tokensDelegatedId.length - 1) {
...
export function updateDelegatorTotalDelegation(
  ...
  for (let i = 0; i < tokens.length; i++) {
    let operation = action == "delegated" ? "add" : "withdraw";
...
export function updateClustersInfo(
  ...
  if (clusters.length > 0) {
    for (let i = 0; i < clusters.length; i++) {
      ...
      let unregisterBlock = cluster.clusterUnregistersAt as BigInt;
      ...
      if (unregisterBlock.gt(BIGINT_ZERO) && blockNumber.ge(unregisterBlock)) {
        cluster.status = STATUS_NOT_REGISTERED;
        cluster.clusterUnregistersAt = BIGINT_ZERO;
